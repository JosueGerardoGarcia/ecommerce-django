...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail_subject = 'Por favor activa tu cuenta en PethShop'
...
# Lógica para crear y activar la cuenta mediante un link
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, 'Tu cuenta a sido activada')
        return redirect('login')

    else:
        messages.error(request, 'La activación no fue completada')
